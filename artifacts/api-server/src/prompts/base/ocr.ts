...
export function buildOcrPrompt(ctx: OcrPromptContext): string {
  ...
  const segmentHints = seg
    ? `\nDicas de terminologia para este segmento (${seg.label}):
- Receitas são chamadas de: ${seg.terminologia.receita}
- Despesas são chamadas de: ${seg.terminologia.despesa}
- Documentos comuns: ${seg.exemplosDocumentos.join(", ")}
`
    : "";
...
- Valores positivos para ${seg ? seg.terminologia.receita + "s" : "vendas/receitas/recebimentos"}. Negativos para ${seg ? seg.terminologia.despesa + "s" : "despesas"}.
